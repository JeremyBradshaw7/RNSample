...
#
# Note: before a production release for the app store we want to take the steps documented here: https://facebook.github.io/react-native/docs/running-on-device.html#building-your-app-for-production
# We could keep these changes in a STASH and apply via, for example if we ensure the stash is always the first alphanumerically:
# git -c diff.mnemonicprefix=false -c core.quotepath=false -c credential.helper=sourcetree stash apply stash@{0} 
# or do this:
# git apply PRODUCTION.diff
# We might need different exportOptionsPlist files for deployment to testers thru TF/AC and deployment to App Store
...
# at some point started creating Competence.ipa rather than ccf.ipa!
# cp ./ios/build/Build/Outputs/Competence.ipa ./ios/build/Build/Outputs/ccf.ipa
...
#     -only-testing:TEST-IDENTIFIER                            constrains testing by specifying tests to include#     -test-timeouts-enabled YES|NO                            enable or disable test timeout behavior
...
#     -maximum-test-execution-time-allowance SECONDS           the maximum execution time an individual test is given to execute, regardless of the test's preferred allowance, and excluding other tests
